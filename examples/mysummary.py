...
def train():
  ...
  train_writer = tf.summary.FileWriter(FLAGS.log_dir + '/train',
                                                 sess.graph, session=sess)
  test_writer = tf.summary.FileWriter(FLAGS.log_dir + '/test', session=sess)
...
def tf13_graph_version():
    g = tf.compat.v1.Graph()
    with g.as_default():
        step = tf.Variable(0, dtype=tf.int64)
        step_update = step.assign_add(1)
        writer = tf.contrib.summary.create_file_writer("/tmp/tensorflow/mnist")
        all_summary_ops = list()
        with writer.as_default():
            sop = tf.contrib.summary.scalar("my_metric", 0.5, step=step)
            all_summary_ops.append(sop)
    ...
    #with tf.contrib.Session(graph=g) as sess:
...
def tf_graph_version():
    ...
    def my_func(step):
        with writer.as_default():
            ...
            tf.contrib.summary.scalar("my_metric", 0.5, step=step)
